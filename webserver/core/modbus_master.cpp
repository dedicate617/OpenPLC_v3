...
void applyLegacyHoldingConfig(struct MB_device *device)
{
...
void parseConfig()
{
        ...
        while (getline(cfgfile, line))
        {
            ...
            if (line_str[0] != '#' && strlen(line_str) > 1)
            {
                ...
                else if (!strncmp(line_str, "device", 6))
                {
                    ...
                    {
                        int block_idx = -1;
                        if (sscanf(functionType, "Holding_Registers_Read_Block%d_Start", &block_idx) == 1)
                        {
    ...
    {
        applyLegacyHoldingConfig(&mb_devices[i]);
    }
