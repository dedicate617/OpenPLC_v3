...
def get_hr_register_blocks(conn, dev_id, reg_type):
    cur = conn.cursor()
...
def parse_hr_blocks(raw_blocks):
...
def store_hr_register_blocks(conn, dev_id, reg_type, blocks):
    cur = conn.cursor()
    ...
    if (conn != None):
        try:
            ...
            for row in rows:
                ...
                hr_read_blocks = get_hr_register_blocks(conn, row[0], 'HR_READ')
                hr_write_blocks = get_hr_register_blocks(conn, row[0], 'HR_WRITE')
        ...
        if (flask.request.method == 'GET'):
            ...
            if (conn != None):
                try:
                    ...
                    hr_read_blocks = parse_hr_blocks(hr_read_blocks_raw)
                    hr_write_blocks = parse_hr_blocks(hr_write_blocks_raw)
                    store_hr_register_blocks(conn, dev_id_db, 'HR_READ', hr_read_blocks)
                    store_hr_register_blocks(conn, dev_id_db, 'HR_WRITE', hr_write_blocks)
                    conn.commit()
                    cur.close()
                    conn.close()
        ...
        if (flask.request.method == 'GET'):
            ...
            if (conn != None):
                try:
                    ...
                    row = cur.fetchone()
                    cur.close()
                    hr_read_blocks = get_hr_register_blocks(conn, dev_id, 'HR_READ')
                    hr_write_blocks = get_hr_register_blocks(conn, dev_id, 'HR_WRITE')
            ...
            if (conn != None):
                try:
                    ...
                    hr_read_blocks = parse_hr_blocks(hr_read_blocks_raw)
                    hr_write_blocks = parse_hr_blocks(hr_write_blocks_raw)
                    store_hr_register_blocks(conn, devid_db, 'HR_READ', hr_read_blocks)
                    store_hr_register_blocks(conn, devid_db, 'HR_WRITE', hr_write_blocks)
                    conn.commit()
                    cur.close()
                    conn.close()
