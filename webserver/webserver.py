...
from protocol_dsl_compiler import compile_session
...
PROTOCOL_DSL_SESSION_DIR = (BASE_DIR / "protocol_dsl_sessions").resolve()
...
@csrf.exempt
@app.route('/api/protocol-dsl/sessions', methods=['GET'])
def api_protocol_dsl_sessions():
    if (flask_login.current_user.is_authenticated == False):
        return flask.jsonify({"error": "unauthorized"}), 401

    PROTOCOL_DSL_SESSION_DIR.mkdir(parents=True, exist_ok=True)
    sessions = []
    for f in sorted(PROTOCOL_DSL_SESSION_DIR.glob('*.json')):
        if str(f.name).endswith('.compiled.json'):
            continue
        compiled = f.with_name(f.stem + '.compiled.json')
        sessions.append({
            "session": f.name,
            "compiled": compiled.name if compiled.exists() else None
        })
    return flask.jsonify({"sessions": sessions})


@csrf.exempt
@app.route('/api/protocol-dsl/compile', methods=['POST'])
def api_protocol_dsl_compile():
    if (flask_login.current_user.is_authenticated == False):
        return flask.jsonify({"error": "unauthorized"}), 401
    ...
    session_name = payload.get('session_name', 'session')
    if not isinstance(session_name, str) or len(session_name.strip()) == 0:
        return flask.jsonify({"error": "invalid_session_name"}), 400

    safe_name = ''.join(ch for ch in session_name if ch.isalnum() or ch in ('_', '-', '.')).strip('.')
    if safe_name == '':
        return flask.jsonify({"error": "invalid_session_name"}), 400
    ...
    try:
        ...
        return flask.jsonify({"error": "compile_failed", "details": str(exc)}), 400

    PROTOCOL_DSL_SESSION_DIR.mkdir(parents=True, exist_ok=True)
    src_file = PROTOCOL_DSL_SESSION_DIR / (safe_name + '.json')
    compiled_file = PROTOCOL_DSL_SESSION_DIR / (safe_name + '.compiled.json')

    src_file.write_text(json.dumps(session, ensure_ascii=False, indent=2) + "\n", encoding='utf-8')
    compiled_file.write_text(json.dumps(compiled, ensure_ascii=False, indent=2) + "\n", encoding='utf-8')

    return flask.jsonify({
        "status": "ok",
        "session_file": str(src_file),
        "compiled_file": str(compiled_file),
        "tx_ir_tokens": len(compiled.get('tx_ir', [])),
        "rx_ir_tokens": len(compiled.get('rx_ir', []))
    })
